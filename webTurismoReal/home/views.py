...
def home_reserva_confirmacion(request, id):
    ...
    return HttpResponse(
        render(
            request,
            'user/bookdpto.html', {'detalle_dpto': detalle_dpto,'all_tours':all_tours,'all_transp':all_transp}
        )
    )
...
def home_reserva(request):
    if request.method == "POST":
        #--------------------------------- SERVICIOS ---------------------------------
        detalle_tp_id = request.POST['detalletpid']
        detalle_tp = DetalleTP.objects.all().get(id=detalle_tp_id)
        detalle_tour_id = request.POST['tourid']
        detalle_tour = Tour.objects.all().get(id=detalle_tour_id)
        #--------------------------------- SERVICIOS ---------------------------------
        ...
        reservation.detalle_tp = detalle_tp_object
        reservation.tour = detalle_tour_object
        #-----------------------------------------------------------------------------------------------------------
